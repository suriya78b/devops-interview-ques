...
NetworkPolicy is a Kubernetes resource that allows you to define fine-grained network access rules for pods — essentially a pod-level firewall.
By default in Kubernetes:
All pods can talk to all other pods (in any namespace) on any port → completely open network.
...
- podSelector → Which pods does this policy apply to? (the "targets")
- ingress → Who is allowed to send traffic to these pods?
- egress → Which destinations these pods are allowed to send traffic to?
...
- Deny does not override allow — if any policy allows a particular connection, the traffic is permitted.
- Only if no policy allows the connection → traffic is denied.
...
### Security
- Mutual TLS (mTLS) → automatic encryption + service identity authentication
...
### Popular Meshes
- Istio (Envoy proxy) → Most feature-rich, complex, widely used in enterprises.
- Linkerd → Lightweight, simple, fast adoption.
- Cilium Service Mesh → No sidecar (eBPF-based), high performance.

### How It Works (Quick Flow)
- Install mesh control plane.
- Enable sidecar injection in namespace (istio-injection=enabled).
- Deploy apps → pods get proxy automatically.
...
- Kubernetes Network Policies – Built-in L3/L4 (IP + port) firewall.
- Service Mesh (e.g., Istio, Linkerd, Consul) – Advanced L7 security with mutual TLS (mTLS), authorization, and observability.

Refer Network policies [here](#7-Network-policy-in-K8s) and Service Mesh here.
...
## 11. Explain how you’d implement runtime security for containers using tools like Falco or eBPF.
...
## 18. New pods stay in “Pending” but the cluster autoscaler is not creating new nodes. What could be wrong?
## 19. A pod is running but refuses TCP connections – how do you debug?
...
## 25. Explain how Kubernetes schedules a pod from request → running state.
...
## 30. Pod in CrashLoopBackOff — how do you debug?
## 31. Deployment vs StatefulSet vs DaemonSet — real use cases?
## 32. How do you expose a service externally?
33. How do ConfigMaps and Secrets work?
34. RollingUpdate vs Recreate strategy — when to use which?
